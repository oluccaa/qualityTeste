...
export const SupabaseQualityService: IQualityService = {
  getManagedPortfolio: async (analystId) => {
    try {
      ...
      console.error("[QualityService] Erro ao carregar portfólio:", err);
  ...
  submitVeredict: async (user, file, status, reason) => {
    ...
    const newMetadata = { 
      ...
      // Incrementa step se for aprovação final
      currentStep: status === QualityStatus.APPROVED ? 5 : file.metadata?.currentStep
    };
    ...
    await logAction(
      user, 
      isNewRejection ? 'QUALITY_VEREDICT_REJECTED' : 'QUALITY_VEREDICT_APPROVED', 
      file.name, 
      'DATA', 
      isNewRejection ? 'WARNING' : 'INFO'
    );
  }
};
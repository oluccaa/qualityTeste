...
import { ChevronLeft, ChevronRight, Settings, LogOut, LayoutDashboard, Library } from 'lucide-react';
...
export const SidebarClient: React.FC<SidebarClientProps> = ({ user, role, isCollapsed, onToggle, onLogout, onNavigateToSettings }) => {
  ...
  const isActive = (path: string, exact = false) => {
    ...
    // Se o caminho definido no menu tem query params, a comparação deve ser exata
    if (path.includes('?')) {
      return currentFull === path;
    }
    
    // Se for rota raiz do dashboard e estivermos na biblioteca, não deve ativar o "Início"
    if (path === '/client/dashboard' && location.search.includes('view=')) {
      return false;
    }
        ...
        <div className={`flex items-center p-2.5 bg-white/5 border border-white/10 rounded-xl gap-3 transition-all ${isCollapsed ? 'justify-center' : ''}`}>
          ...
          {!isCollapsed && (
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[9px] text-orange-500/80 uppercase font-black tracking-tighter">PARCEIRO VITAL</p>
            </div>
          )}
        </div>
      </div>

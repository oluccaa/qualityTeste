...
export const SidebarClient: React.FC<SidebarClientProps> = ({ user, isCollapsed, onToggle, onLogout, onNavigateToSettings }) => {
    ...
    <aside className={`hidden md:flex flex-col bg-white border-r border-slate-200 shadow-sm z-[60] relative transition-all duration-500 ${isCollapsed ? 'w-20' : 'w-72'}`}>
      ...
      <div className={`h-24 flex items-center shrink-0 transition-all ${isCollapsed ? 'justify-center' : 'px-8'}`}>
        <img 
          src={LOGO_URL} 
          alt="Aços Vital" 
          className={`${isCollapsed ? 'h-7' : 'h-16'} transition-all object-contain`} 
      ...
      <div className="p-4 bg-slate-50 border-t border-slate-100 space-y-2">
        {!isCollapsed && (
          ...
          >
            <Settings size={18} />
            <span className="text-sm font-semibold tracking-tight">{t('menu.settings')}</span>
          </button>
        )}
        ...
        <div className={`mt-4 flex items-center p-2 rounded-xl gap-3 transition-all ${isCollapsed ? 'justify-center' : 'bg-white border border-slate-200'}`}>
          ...
          {!isCollapsed && (
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Parceiro</p>
            </div>
          )}
        </div>
      </div>

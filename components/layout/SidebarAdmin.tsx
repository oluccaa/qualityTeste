...
import { getAdminMenuConfig } from '../../config/navigation.ts';
...
export const SidebarAdmin: React.FC<SidebarAdminProps> = ({ 
  ...
  const menuSections = getAdminMenuConfig(t);
    ...
    <aside className={`hidden md:flex flex-col bg-white border-r border-slate-200 shadow-sm z-[60] relative transition-all duration-500 ${isCollapsed ? 'w-20' : 'w-72'}`}>
                ...
                return (
                  <Link 
                    ...
                    className={`flex items-center transition-all duration-200 group relative ${
                      ...
                      active 
                      ? 'bg-slate-100/80 text-slate-900 border-l-4 border-[#b23c0e]' 
                      : 'text-slate-500 hover:bg-slate-50 border-l-4 border-transparent'
                    }`}
                  >
                    <Icon size={18} strokeWidth={active ? 2.5 : 2} className={active ? 'text-[#b23c0e]' : 'text-slate-400 group-hover:text-slate-600'} />
      ...
      <div className="p-4 bg-slate-50 border-t border-slate-100 space-y-2">
        ...
        <div className={`mt-4 flex items-center p-2 rounded-xl gap-3 transition-all ${isCollapsed ? 'justify-center' : 'bg-white border border-slate-200'}`}>
          <div className="w-10 h-10 rounded-lg bg-[#b23c0e] flex items-center justify-center text-white shrink-0 font-bold">
            {user?.name?.charAt(0)}
          </div>
          {!isCollapsed && (

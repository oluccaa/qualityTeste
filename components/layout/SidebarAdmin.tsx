      ...
      <nav className="flex-1 py-6 space-y-1 overflow-y-auto custom-scrollbar px-3">
        {menuSections.map((section, idx) => (
          <div key={idx} className="mb-6">
            {!isCollapsed && (
              <div className="mb-2 px-3 text-[9px] font-black uppercase tracking-[3px] text-orange-500/60">
                {section.title}
              </div>
            )}
      ...
      <div className="p-4 border-t border-white/5 bg-black/20 space-y-3">
        {!isCollapsed && (
          ...
          >
            ...
            <span className="text-sm font-semibold truncate">Configurações Master</span>
          </button>
        )}
        ...
        <div className={`flex items-center p-2.5 bg-white/5 border border-white/10 rounded-xl gap-3 transition-all ${isCollapsed ? 'justify-center' : ''}`}>
          ...
          {!isCollapsed && (
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[9px] text-[#b23c0e] uppercase font-black tracking-tighter">{user?.email || 'N/A'}</p>
            </div>
          )}
        </div>
      </div>
